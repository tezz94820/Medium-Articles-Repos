...
app.get("/books/:book_id", (req, res) => {
    ...
    if (!book.eTag) {
        ...
        books.forEach(bookItem => {
            bookItem.eTag = book.eTag;
        });
    }

    // Extract the If-None-Match header as ETag from req
    const clientETag = req.headers['if-none-match'];

    // If client's ETag matches the server's ETag, return 304 Not Modified
    if (clientETag === book.eTag) {
        return res.status(304).end(); // Resource not modified
    }
